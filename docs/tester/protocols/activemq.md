...
ActiveMQ 协议支持为 Ryze 测试框架提供了与 Apache ActiveMQ 消息中間件进行交互的能力。支持队列和主题两种消息模式，可以进行消息发送、接收和处理的完整测试。
...
| 配置项 | 类型 | 默认值 | 必需 | 描述 |
|-------|------|--------|------|------|
| broker.url | String | - | ✅ | ActiveMQ Broker URL (tcp://host:port) |
| username | String | - | ❌ | ActiveMQ 用户名 |
| password | String | - | ❌ | ActiveMQ 密码 |
...
| 配置项 | 类型 | 默认值 | 必需 | 描述 |
|-------|------|--------|------|------|
| message | Object | - | ✅ | 发送的消息 (JSON/字符串/数字) |

> **配置优先级**: 取样器配置 > ActiveMQ 默认配置
>
> **注**: *表示 queue 和 topic 选择一个，queue 优先级高于 topic
